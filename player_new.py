...
class Player:
    """
    This is the 10000 bugs.
    For Max it hides from beginning then hits in the last turn
    ...
    def max_strategy(self):
        ...
        if self.turn_num <= 9:
            ret_d[self.v_col] = self.BOUNDARY  # to rule out SumNeg

        elif self.turn_num == 10:
            next_unique_max = self.get_next_unique_max()
            current_max = self.get_current_unique_max()
            if 'Max' in self.enemy_col_cond:
                ret_d[self.enemy_col_cond['Max']] = next_unique_max
                next_next_unique_max = self.next_after(next_unique_max, -1)
                ret_d[self.v_col] = next_next_unique_max
            else:
                if current_max > 0 and current_max not in self.data[self.v_col]:
                    max_col = None
                    for k in self.data:
                        if current_max in self.data[k]:
                            max_col = k

                    if max_col:
                        ret_d[max_col] = current_max        # make current max non unique first
                    else:
                        raise ValueError('max_col none')

                ret_d[self.v_col] = next_unique_max
    ...
    def min_strategy(self):
        ...
        if self.turn_num <= 9:
            ...
            next_unique_min = self.get_next_unique_min()
            current_min = self.get_current_unique_min()
            if 'Min' in self.enemy_col_cond:
                ret_d[self.enemy_col_cond['Min']] = next_unique_min
                next_next_unique_min = self.next_after(next_unique_min, 1)
                ret_d[self.v_col] = next_next_unique_min
            else:
                if current_min > 0 and current_min not in self.data[self.v_col]:
                    min_col = None
                    for k in self.data:
                        if current_min in self.data[k]:
                            min_col = k

                    if min_col:
                        ret_d[min_col] = current_min        # make current min non unique first
                    else:
                        raise ValueError('min_col none')

                ret_d[self.v_col] = next_unique_min

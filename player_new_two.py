...
class Player:
    ...
    def max_strategy(self):
        ...
        if self.turn_num == 10:
            ...
            if current_max not in self.data[self.v_col]:
                # this means the opponent has guessed our logic, so we move by 2
                next_max = self.next_after(next_max, -1)
                ret_d[self.v_col] = self.next_after(next_max, -1)
            else:
                ret_d[self.v_col] = self.next_after(next_max, -1)
    ...
    def min_strategy(self):
        ...
        if self.turn_num == 10:
            ...
            if current_min not in self.data[self.v_col]:
                # this means the opponent has guessed our logic, so we move by 2
                next_min = self.next_after(next_min, 1)
                ret_d[self.v_col] = self.next_after(next_min, 1)
            else:
                ret_d[self.v_col] = self.next_after(next_min, 1)
        ...
        for k in sorted(self.data.keys()):
            if k not in ret_d:  # only insert values if it is not already added from strategy functions
                ...
                if self.turn_num == 1 or current_sum == 0:
                    if count % 2:   # alternating 1023/-1023 to cover both cases
